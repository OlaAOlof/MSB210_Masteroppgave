...
Energi_2019_Vestland <- suppressWarnings(read_csv("data/Energikarakterer_2019.csv"))
Energi_2020_Vestland <- suppressWarnings(read_csv("data/Energikarakterer_2020.csv"))
Energi_2021_Vestland <- suppressWarnings(read_csv("data/Energikarakterer_2021.csv"))
Energi_2022_Vestland <- suppressWarnings(read_csv("data/Energikarakterer_2022.csv"))
```
...
Energi_2019_Vestland <- Energi_2019_Vestland[Energi_2019_Vestland$Knr %in% c(4601, 4623, 4624, 4626, 4627, 4628, 4630, 4631, 4632), ]

Energi_2020_Vestland <- Energi_2020_Vestland[Energi_2020_Vestland$Knr %in% c(4601, 4623, 4624, 4626, 4627, 4628, 4630, 4631, 4632), ]

Energi_2021_Vestland <- Energi_2021_Vestland[Energi_2021_Vestland$Knr %in% c(4601, 4623, 4624, 4626, 4627, 4628, 4630, 4631, 4632), ]

Energi_2022_Vestland <- Energi_2022_Vestland[Energi_2022_Vestland$Knr %in% c(4601, 4623, 4624, 4626, 4627, 4628, 4630, 4631, 4632), ]
```
...
```{r eval=FALSE}
tmp <- Ambita_Oppdatert %>% 
  filter(Kommunenr == 4628)
```

```{r eval=FALSE}
tmp <- tmp[!duplicated(tmp$InternidDokument), ]
```

```{r eval=FALSE}
tmp2 <- Ambita %>% 
  filter(Bygningsnr == 175830898)
```

```{r eval=FALSE}
tmp2 <- tmp2[!duplicated(tmp2$InternidDokument), ]
```

## Lager et temp-datasett for andelsnummer

```{r}
temp_andel <- Ambita_Oppdatert %>% 
  select(Andelsnr, Kommunenr, Gardsnr, Bruksnr, EiendomNordSor, EiendomOstVest, InternIdBruksenhet, Bygningsnr, Dokumentnr)
```
...
# Splitter matrikkel i Analyse-datasettet
...
# Legger inn Andelsnummer i Analyse datasettet

```{r}
colnames(temp_andel)[colnames(temp_andel) == "Kommunenr"] <- "Knr"
colnames(temp_andel)[colnames(temp_andel) == "Gardsnr"] <- "Gnr"
colnames(temp_andel)[colnames(temp_andel) == "Bruksnr"] <- "Bnr"
colnames(temp_andel)[colnames(temp_andel) == "Andelsnr"] <- "Andelsnummer"
colnames(temp_andel)[colnames(temp_andel) == "Bygningsnr"] <- "Bygningsnummer"
```

```{r}
Analyse <- transform(Analyse, Knr = as.numeric(Knr))
Analyse <- transform(Analyse, Gnr = as.numeric(Gnr))
Analyse <- transform(Analyse, Bnr = as.numeric(Bnr))
```

```{r}
TEST <- inner_join(Analyse, temp_andel, by = c("Dokumentnr", "Bygningsnummer", "Knr", "Gnr", "Bnr"))
```

```{r}
unique_analyse <- Analyse %>%
  distinct(Dokumentnr, .keep_all = TRUE)


unique_temp_andel <- temp_andel %>%
  distinct(Dokumentnr, .keep_all = TRUE)

Analyse <- inner_join(unique_analyse, unique_temp_andel, by = c("Dokumentnr"))
```

```{r}
Analyse <- Analyse %>% 
  select(-Bygningsnummer.y, -EiendomOstVest.y, -EiendomNordSor.y, -Bnr.y, -Gnr.y, -Knr.y)
```

```{r}
colnames(Analyse)[colnames(Analyse) == "EiendomOstVest.x"] <- "EiendomOstVest"
colnames(Analyse)[colnames(Analyse) == "EiendomNordSor.x"] <- "EiendomNordSor"
colnames(Analyse)[colnames(Analyse) == "Bygningsnummer.x"] <- "Bygningsnummer"
colnames(Analyse)[colnames(Analyse) == "Bnr.x"] <- "Bnr"
colnames(Analyse)[colnames(Analyse) == "Gnr.x"] <- "Gnr"
colnames(Analyse)[colnames(Analyse) == "Knr.x"] <- "Knr"
```

# Fjerner feilregistrerte observasjoner på Antall Rom, Bad og WC

**Det er snakk om 21 observasjoner som ser ut til å vere feilregistrert eller har manglande registrering på variablene Bad og WC. Vi fjerner disse**
...
```{r}
Analyse_sf$EtasjeSeksjon[Analyse_sf$EtasjeSeksjon == 'NA'] <- "Ikke Oppgitt"
```

```{r}
Analyse_sf$Snr[Analyse_sf$Snr == 'NA'] <- 0
```
...
# Rydder ut observasjoner vi ikke kan bruke

**Vi fjerner Bruksareal.Enhet som er lik 0. Dette kommer av at Ambita ikke alltid skiller mellom hele leilighetskomplekset og hver enkelt leilighet.**

## Analyse

```{r}
Analyse <- transform(Analyse, Bruksareal.Enhet = as.numeric(Bruksareal.Enhet))
```

```{r}
Analyse$Bruksareal.Enhet[(876)] <- 79
Analyse$Bruksareal.Enhet[(877)] <- 62
Analyse$Bruksareal.Enhet[(878)] <- 81
Analyse$Bruksareal.Enhet[(879)] <- 60
```

```{r}
Analyse <- Analyse %>% 
  filter(Bruksareal.Enhet > 0)
```

```{r}
Analyse <- Analyse %>% 
  mutate(pm2 = P/Bruksareal.Enhet) %>% 
  relocate(pm2, .before = Alder)
```
...
```{r}
Analyse_sf$Bruksareal.Enhet[(876)] <- 79
Analyse_sf$Bruksareal.Enhet[(877)] <- 62
Analyse_sf$Bruksareal.Enhet[(878)] <- 81
Analyse_sf$Bruksareal.Enhet[(879)] <- 60
```
...
## Skriver ut Excel som skal til Liv

```{r}
#write.xlsx(Analyse, "data/Datasett.xlsx")
```

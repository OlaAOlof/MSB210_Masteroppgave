...
```{r}
Dato <- Ambita %>% 
  select(DokumentAr, Dokumentnr, DokumentDato)
```
...
### Prøver å fikse koordinater (Dette var visst ikkje nødvendig)

Dette viste seg å ikke være nødvendig. Lar det stå inntil videre om det skulle bli nødvendig, men krysser ut koden, så den ikke blir kjørt fremover, så lenge vi ikke ønsker og/eller trenger det.
...
```{r}
#temp <- Ambita %>% 
#  select(EiendomNordSor, EiendomOstVest, DokumentDato, Dokumentnr)
```

```{r}
#test <- inner_join(Analyse, temp, by = c("Dokumentnr", "DokumentDato"))
```
...
# Slår sammen nye koordinater?

```{r}
# Finner match

#matching_indices <- match(Ambita_Gammel$Dokumentnr, Ambita$Dokumentnr)

# Erstattet EiendomNordSor og EiendomOstVest i Ambita basert på match
#Ambita_Gammel$EiendomNordSor[!is.na(matching_indices)] <- Ambita$EiendomNordSor[matching_indices[!is.na(matching_indices)]]
#Ambita_Gammel$EiendomOstVest[!is.na(matching_indices)] <- Ambita$EiendomOstVest[matching_indices[!is.na(matching_indices)]]
```
...
```{r}
#Analyse <- transform(Analyse, EiendomNordSor = as.numeric(EiendomNordSor))
```

```{r}
#Analyse$NordSor <- conv_unit(Analyse$EiendomNordSor, from = "deg_min_sec", to = "dec_deg")
```

```{r}
Test_NordSor <- conv_unit("67 39 596.3", from = "deg_min_sec", to = "dec_deg")
```

```{r}
Test_OstVest <- conv_unit("3 9 036.06", from = "deg_min_sec", to = "dec_deg")
```

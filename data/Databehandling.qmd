...
```{r}
#Analyse_sf <- transform(Analyse_sf, EiendomNordSor = as.numeric(EiendomNordSor))
```

```{r}
#Analyse_sf$NordSor <- conv_unit(Analyse_sf$EiendomNordSor, from = "deg_min_sec", to = "dec_deg")
```

```{r}
#Analyse_sf$OstVest <- conv_unit(Analyse_sf$EiendomOstVest, from = "deg_min_sec", to = "dec_deg")
```

```{r}
#Test_NordSor <- conv_unit("67 03 281.14", from = "deg_min_sec", to = "dec_deg")
```

```{r}
#Test_OstVest <- conv_unit("-129 33.33", from = "deg_min_sec", to = "dec_deg")
```

...
```{r}
Bolig_pm2 <- Bolig %>% 
  select(P, BRA.Bolig, BRA.Tot, DokumentAr, Knr)
```

```{r}
Blokker_pm2 <- Blokker %>% 
  select(P, Bruksareal.Enhet, BRA.Tot, DokumentAr, Knr)
```

```{r}
Bolig_pm2 <- Bolig_pm2 %>% 
  mutate(pm2 = P/BRA.Tot)
```
...
Gjennomsnittlig_kvartalspris <- Analyse %>%
  ...
  summarize(Gjenomsnittspris = mean(P))
...
ggplot(Gjennomsnittlig_kvartalspris, aes(x = YearQuarter, y = Gjenomsnittspris, group = 1)) +
  ...
  labs(x = "År og kvartal", y = "Gjennomsnittlig boligpris", title = "Gjennomsnittlig kvartalspris med strømkrisen i grå sone")
```

...
# Mini oppdatering av data

```{r eval=FALSE}
Region_2019 <- Analyse_sf %>% 
  filter(Knr > 4601, DokumentAr == 2019)
```

```{r eval=FALSE}
Region_2022 <- Analyse_sf %>% 
  filter(Knr > 4601, DokumentAr == 2022)
```
...
```{r}
Bergen_sf$EL.Kar.Original <- as.factor(Bergen_sf$EL.Kar.Original) # Character -> Factor
Bergen_sf$EL.Kar.Original <- relevel(Bergen_sf$EL.Kar.Original, ref = "A") # Referanseverdi
```

```{r}
Bergen_sf$O.Karak <- as.factor(Bergen_sf$O.Karak) # Character -> Factor
Bergen_sf$O.Karak <- relevel(Bergen_sf$O.Karak, ref = "Green") # Referanseverdi
```

```{r}
Bergen_sf$YearQuarter <- as.factor(Bergen_sf$YearQuarter) # Character -> Factor
Bergen_sf$YearQuarter <- relevel(Bergen_sf$YearQuarter, ref = "2022Q4") # Referanseverdi
```

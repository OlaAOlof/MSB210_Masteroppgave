...
```{r eval=FALSE}
Modell_Bergen_log <- "log(pm2) ~ EL.Kar.Original + log(Alder+1) + O.Karak + log(BLEnergikWh) + MatValg + log(AntallRom) + log(AntallBad) + log(AntallWC) + HarHeis + YearQuarter + log(Bruksareal.Enhet) + log(dist_cbd_km)"
```
...
```{r eval=FALSE}
Modell_Generell_log <- "log(pm2) ~ EL.Kar.Original + log(Alder+1) + O.Karak + log(BLEnergikWh) + MatValg + log(AntallRom) + log(AntallBad) + log(AntallWC) + HarHeis + YearQuarter + log(Bruksareal.Enhet) + log(dist_cbd_km)"
```

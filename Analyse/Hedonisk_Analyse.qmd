...
```{r}
# SAR model
sar_model <- lagsarlm(P ~ Alder + B.Ar + EL.Kar + O.Karak_num + BLEnergikWh + MatValg_num + AntEiendom + EiendomAreal + TotBoliger + BRA.Bolig + Etasje_num + dist_cbd_km, data = Analyse_sf, listw = Analyse_sf_W3, zero.policy = TRUE)

# SEM model
sem_model <- errorsarlm(sale_price ~ energy_certificates + other_covariates, data = Analyse_spdf, listw = w, zero.policy = TRUE)

# SDM model
sdm_model <- sacsarlm(sale_price ~ energy_certificates + other_covariates, data = Analyse_spdf, listw = w, zero.policy = TRUE)

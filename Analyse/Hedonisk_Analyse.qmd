...
Analyse_sf$EL.Kar.Original <- relevel(Analyse_sf$EL.Kar.Original, ref = "G") # Referanseverdi
```
...
```{r}
summary(sar_model_kommuner_k3)
```

```{r}
summary(sar_model_kommuner_k10)
```

```{r}
summary(sar_model_generell_k3)
```

```{r}
summary(sar_model_generell_k10)
```

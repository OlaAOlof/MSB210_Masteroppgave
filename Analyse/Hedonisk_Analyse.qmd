...
modBlokkEnergi <- "pm2 ~ EL.Kar.Original + O.Karak + BLEnergikWh + MatValg + Alder + YearQuarter"
OLSBlokkEnergi <- lm(modBlokkEnergi, data = Blokker_sf)
summary(OLSBlokkEnergi)
AIC(OLSBlokkEnergi)
BIC(OLSBlokkEnergi)
...
modBlokkEnergi <- "pm2 ~ EL.Kar.Original + O.Karak + BLEnergikWh + MatValg + Alder + YearQuarter + Kommune"
OLSBlokkEnergi <- lm(modBlokkEnergi, data = Blokker_sf)
summary(OLSBlokkEnergi)
AIC(OLSBlokkEnergi)
BIC(OLSBlokkEnergi)
...
BergenEnergi <- "pm2 ~ EL.Kar.Original + O.Karak + BLEnergikWh + MatValg + Alder + YearQuarter"
...
```{r}
Modell_Bergen <- "pm2 ~ EL.Kar.Original + Alder + O.Karak + BLEnergikWh + MatValg + AntallRom + AntallBad + AntallWC + HarHeis + YearQuarter + dist_cbd_km"
```

```{r eval=FALSE}
Modell_Kommuner <- "pm2 ~ EL.Kar.Original + Alder + O.Karak + BLEnergikWh + MatValg + AntallRom + AntallBad + AntallWC + HarHeis + YearQuarter + Kommune + dist_cbd_km"
```

```{r}
Modell_Generell <- "pm2 ~ EL.Kar.Original + Alder + O.Karak + BLEnergikWh + MatValg + AntallRom + AntallBad + AntallWC + HarHeis + YearQuarter + dist_cbd_km"
```
...
```{r}
R2_SEM_Bergen_K3 <- sem(Modell_Bergen,
                        data = Bergen_sf)
```

```{r}
R2_SEM_Bergen_K3 <- sem(sem_model_Bergen_k3,
                        data = Bergen_sf)
```

```{r}
summary(sem_model_Bergen_k3, rsq = TRUE, fit.measures = TRUE, standardized = TRUE)
```

#### R2, SEM, Bergen, K10

```{r}
R2_SEM_Bergen_K3 <- sem(Modell_Bergen, data = Bergen_sf)
```

```{r}
summary(test_sem, rsq = TRUE, fit.measures = TRUE, standardized = TRUE)
```

### Morans I av SEM-modell (Prøver å finne ut av)
...
slx_model_Bergen_k3 <- lmSLX(Modell_Bergen,
                                 data = Bergen_sf,
                                 listw = Analyse_W3_red,
                                 Durbin = as.formula(~ EL.Kar.Original + Alder + O.Karak + BLEnergikWh + MatValg + AntallRom + AntallBad + AntallWC + HarHeis + YearQuarter + dist_cbd_km)
                               )
```
...
```{r eval=FALSE}
lmtest::lrtest(sar_model_Bergen_k3, sac_model_Bergen_K3, sem_model_Bergen_k3, slx_model_Bergen_k3, SDM_modell_Bergen_K3)
```

## Tester Bergen, K10

```{r eval=FALSE}
lmtest::lrtest(sar_model_Bergen_k10, sac_model_Bergen_K10, sem_model_Bergen_k10, slx_model_Bergen_k10, SDM_modell_Bergen_K10)
```

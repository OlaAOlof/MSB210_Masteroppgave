...
mod1 <- "pm2 ~ EL.Kar.Original"
OLS1 <- lm(mod1, data = Blokker_sf)
summary(OLS1)
AIC(OLS1)
BIC(OLS1)
```
...
mod2 \<- "pm2 \~ EL.Kar.Original + Alder"

```{r  Modell 2}
options(scipen = 999)

mod2 <- "I(log(pm2+1)) ~ EL.Kar.Original + log(Alder+1)"
OLS2 <- lm(mod2, data = Blokker)
summary(OLS2)
AIC(OLS2)
BIC(OLS2)
```

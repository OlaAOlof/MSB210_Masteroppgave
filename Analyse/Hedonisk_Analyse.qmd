...
title(main="K nærmeste naboer, k = 3")
```
Funksjonen returnerer en matrise med indeksene til punktene som tilhører settet av de k nærmeste naboene til hverandre.
...
analyse_shape <- st_transform(analyse_shape, crs = st_crs(norge))

# Plotter  MultiPolygon-data
ggplot() + 
  geom_sf(data = norge) +
  coord_sf(crs = st_crs(norge)) +
  theme_bw()
...
# SAR model med høyere tol.solve-verdi
sar_model <- lagsarlm(P ~ Alder + EL.Kar + O.Karak_num + BLEnergikWh + MatValg_num,
                      data = Analyse_sf_subset_num,
                      listw = Analyse_sf_W3,
                      tol.solve = 1e-10)

# SEM model
sem_model <- errorsarlm(P ~ Alder + EL.Kar + O.Karak_num + BLEnergikWh + MatValg_num + AntEiendom + EiendomAreal + BRA.Bolig + Etasje_num + dist_cbd_km, data = Analyse_sf, listw = Analyse_sf_W3, zero.policy = TRUE)

# SDM model
sdm_model <- sacsarlm(P ~ Alder + EL.Kar + O.Karak_num + BLEnergikWh + MatValg_num + AntEiendom + EiendomAreal + BRA.Bolig + Etasje_num + dist_cbd_km, data = Analyse_sf, listw = Analyse_sf_W3, zero.policy = TRUE)

# Tester goodness-of-fit
summary(sar_model)
summary(sem_model)
summary(sdm_model)
lrtest(sar_model, sem_model)
